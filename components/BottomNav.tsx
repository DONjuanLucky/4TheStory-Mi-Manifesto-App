...
const BottomNav: React.FC<BottomNavProps> = ({ activeTab, setActiveTab }) => {
  const tabs = [
    ...
    { id: AppTab.JOURNAL, label: 'Journal', icon: 'M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z' },
  ...
  return (
    <nav id="bottom-nav" className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-xl border-t border-gray-100 z-50 shadow-[0_-10px_30px_rgba(0,0,0,0.02)]">
      <div className="max-w-2xl mx-auto flex justify-around items-center h-20 px-4">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            className={`flex flex-col items-center gap-1.5 transition-all flex-1 ${activeTab === tab.id ? 'text-[#8b7355] scale-110' : 'text-gray-300 hover:text-gray-500'}`}
          >

...
const Onboarding: React.FC<OnboardingProps> = ({ onComplete }) => {
  ...
  const handleNext = () => {
    if (step < 4) setStep(step + 1);
      ...
      finalize(data.persona);
    }
  };

  const finalize = (selectedPersona: PersonaType) => {
    ...
    onComplete(safeTitle, safeGenre, safeWords, selectedPersona);
  };

  return (
    <div className="max-w-2xl mx-auto px-6 py-20 flex flex-col items-center text-center">
      <div className="w-24 h-24 mb-8 flex items-center justify-center animate-in fade-in zoom-in duration-1000">
         <Logo className="w-full h-full" />
      </div>
      <h1 className="font-serif text-4xl mb-6 text-stone-900">Mi Manifesto</h1>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-left">
              {Object.values(PERSONAS).map((p) => (
                <button
                  key={p.id}
                  onClick={() => { 
                    // Directly finalize with the selected persona instead of waiting for state update
                    setData({ ...data, persona: p.id as PersonaType }); 
                    finalize(p.id as PersonaType);
                  }}

...
const CompanionView: React.FC<CompanionViewProps> = ({ project, onOpenEditor, updateProject, lang }) => {
  ...
  // Real-time transcript state for the overlay
  ...
  // Ref to accumulate confirmed messages to save to project history
  ...
  }, [project.messages, isTyping, viewMode]);
  ...
  const updateSoulSummary = async (overrideHistory?: Message[]) => {
    ...
    LATEST MESSAGES (Includes Voice Transcripts): 
    ${historyToAnalyze.slice(-15).map(m => `${m.role.toUpperCase()}: ${m.content}`).join('\n')}
    
    Your goal is to capture the essence, themes, and new plot points discussed.
    
    RETURN A JSON OBJECT WITH TWO FIELDS:
    1. "soulSummary": A 150-word literary summary describing the heart of the story and any new developments.
    2. "lastBreadcrumb": A 20-word specific summary of the last discussion to aid continuity.`;
  ...
  const handleSend = async (content?: string) => {
    ...
    if (newHistory.length % 3 === 0) updateSoulSummary(newHistory);
  };
  ...
  const commitToDraft = async () => {
    ...
    The author has requested to COMMIT TO DRAFT based on the recent conversation.
    
    Transform the following raw conversation/brainstorming session (Voice transcripts and Text) into a POLISHED, STRUCTURED SCENE or CHAPTER.
    
    RULES:
    1. Do not output conversational text. Output ONLY the story content/prose.
    ...
    5. If the user dictated a specific scene, transcribe and polish it into high-quality prose.
    ...
    try {
      ...
      const nextOrder = updatedChapters.length + 1;
      const newChapter: Chapter = {
        id: Math.random().toString(36).substr(2, 9),
        title: `Draft Session ${nextOrder}`,
        content: newProse,
        order: nextOrder
      };
      
      const allChapters = [...updatedChapters, newChapter];
      const totalWords = allChapters.reduce((acc, curr) => acc + curr.content.trim().split(/\s+/).filter(Boolean).length, 0);
  ...
  const saveLiveSessionData = async () => {
    // If we have accumulated messages in the buffer, save them to the project
    if (sessionHistoryBuffer.current.length > 0) {
        const newInteraction: Interaction = {
            id: Math.random().toString(),
            type: 'voice',
            summary: "Voice Session Logged",
            timestamp: new Date(),
            committed: false
        };

        const updatedMessages = [...project.messages, ...sessionHistoryBuffer.current];
        ...
        await updateSoulSummary(updatedMessages);
      ...
      const sessionPromise = ai.live.connect({
        ...
        callbacks: {
          ...
          onmessage: (msg: LiveServerMessage) => {
            ...
            // Commit complete turns to buffer
            ...
            if (userText && currentModelContent.trim()) {
                 sessionHistoryBuffer.current.push({
                    id: Math.random().toString(),
                    role: 'assistant',
                    content: currentModelContent.trim(),
                    timestamp: new Date()
                 });
                 currentModelContent = '';
            }
          ...
          onclose: () => {
            // Seal any remaining buffers
            if (currentUserContent.trim()) {
                sessionHistoryBuffer.current.push({ id: Math.random().toString(), role: 'user', content: currentUserContent.trim(), timestamp: new Date() });
            }
            if (currentModelContent.trim()) {
                sessionHistoryBuffer.current.push({ id: Math.random().toString(), role: 'assistant', content: currentModelContent.trim(), timestamp: new Date() });
            }
            setIsLiveActive(false);
            saveLiveSessionData();
          },
  ...
  const renderContent = () => {
    ...
    return (
      <div className="flex-1 overflow-hidden flex flex-col relative bg-stone-50">
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-6 py-6 space-y-6 scroll-smooth transition-all">
        ...
        {/* FULL SCREEN VOICE AGENT OVERLAY */}
        <AnimatePresence>
          {isLiveActive && (
            <motion.div 
            ...
            >
              ...
              <div className="p-6 flex justify-between items-center border-b border-white/5">
                <div className="flex items-center gap-3">
                   ...
                   <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-stone-400">Live Session</span>
                </div>
                <button 
              ...
              <div className="flex-1 flex flex-col items-center justify-center relative min-h-[300px]">
                 <motion.div 
                    ...
                    className="w-48 h-48 rounded-full bg-[#ea580c] blur-[60px] absolute"
                 />
                 <div className="relative z-10 w-32 h-32 rounded-full border border-white/10 flex items-center justify-center bg-stone-900">
                    <div className={`w-24 h-24 rounded-full bg-[#ea580c] flex items-center justify-center shadow-[0_0_30px_rgba(234,88,12,0.4)] ${isModelSpeaking ? 'animate-pulse' : ''}`}>
              ...
              <div className="h-1/3 bg-black/20 border-t border-white/5 p-6 overflow-y-auto space-y-4">
                 <div className="max-w-2xl mx-auto space-y-4">
                    {sessionTranscript.length === 0 && (
                        <p className="text-center text-stone-600 italic">Conversation will appear here...</p>
                    )}
